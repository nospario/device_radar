...
Echo-device fields in `echo_devices`: `tasks_enabled`, `tasks_interval` (minutes, default 120), `last_tasks_message`. Configured via the Alexa page (`/alexa`) alongside Encourage Mode. The message is built deterministically by `_format_task_reminder()` — no LLM, no weather/time prefix, no motivational filler. Output is just "Tasks due today: A, B, and C. Daily tasks: X, Y, and Z." This guarantees every task is included verbatim, keeps the message tight, and avoids the reliability problems that came from asking a small CPU-only model to list sixteen items without dropping any. The result is passed straight to `speak()`, which chunks it automatically if it exceeds the TTS limit. The `last_tasks_message` timestamp is updated even when both lists are empty so the loop doesn't re-check every minute.
