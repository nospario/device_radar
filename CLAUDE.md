...
Each cycle the outstanding tasks are split into bundles by `_bundle_tasks()` — at most `tasks_max_per_bundle` items each (default 4), keeping the "due today" and "daily reoccurring" groups separate so each bundle can be framed distinctly. One Ollama message is generated per bundle by `_generate_bundle_message()` (target under 300 chars, short motivational framing + all the bundle's task names). `_bundle_covered()` verifies every task appears in the output; if coverage fails or Ollama errors, the bundle falls back to a deterministic `"Richard, daily reoccurring tasks: A, B, and C."` read so nothing is dropped. Because bundles are small and targeted, each message comfortably fits inside a single Simon Says utterance — the global `_chunk_tts()` splitter in `speak()` is a belt-and-braces safety net.
