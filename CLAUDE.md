...
Each hourly reminder (and the on-demand `/habits` command) attaches an **inline keyboard** with one tap-to-complete button per habit. Callback data is `habit:<sha256_16>` of the cleaned description — stable across processes so the scanner (sender) and bot (receiver) don't need shared state. Tapping a button fires `_on_habit_callback` in `bt_telegram.py`: it re-reads today's Daily Note, finds the habit whose hash matches, calls `bt_tasks.complete_habit_by_name()` to mark that single `- [ ]` line as `- [x] ... ✅ YYYY-MM-DD`, then edits the original message in place with the refreshed list + keyboard. When no habits remain, the message becomes "All habits complete for today." and the keyboard is removed. The bot polls with `allowed_updates=["message", "callback_query"]` so button taps flow through.
