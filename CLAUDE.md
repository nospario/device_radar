...
* **Daily Reoccurring Tasks** — default `/home/nospario/ObsidianVaults/Main/3. Todo Lists/Daily Reoccurring Tasks.md`, overridable via `obsidian_daily_tasks_path`. Every uncompleted `- [ ]` item is treated as a daily habit, **except** those whose 📅 date is strictly in the future — when the Obsidian Tasks plugin recurrence fires it creates a new `- [ ]` entry dated for the next occurrence, so filtering out future-dated instances prevents tomorrow's recurrence from being read back as still outstanding today. Items with no date, due today, or overdue are still included.
